...
1. [Overview](#overview)
2. [Feature Extraction/Selection](#features)
3. [Models](#models)
4. [Code](#code)
...
*    I tried a simple 1024-100-2 neural network, tanh activation layer, 20 epochs, weights as for the third GLM model. **AUC: 0.716, 0.683**
...
#### SVM
*    Bootstrapped C-svc SVM  model: sampled the training data 100 times with replacement to obtain the same number of samples as in the given training data. Class was weighted by the fraction of the observations with that outcome. **AUC: 0.761, 0.719**
...
*    A random forest model, with mtry=2, ntree=200, nodesize=10.  The classes were weighted by the fraction of the observations with that outcome. **AUC: 0.717, 0.702**
...
*    The main, essential improvement would be to provide some biological interpretation for the features which characterize preictal/interictal state.  I briefly explored whether some coefficients in the linear model were more informative, but didn't have time to go further.
*    Apart from down-weighing the zero-inflated data, I did not adjust for signal noise in any way.  The challenge organizers suggested that artifacts “such as large amplitude rapid signal transitions” might also appear in signals and can be removed.
*    The feature selection was very simple, partly as a way to learn to play with windowed Fourier transforms in MATLAB.  I would like to go back to the sample MATLAB solution scripts provided in the challenge, which compute many more features (e.g. spectral entropy, etc.).
